...
class aes(object):
  ...
  def __init__(self, k=None, s=None, debug=False, logger=sys.stderr.write):
    self.seed  = s
    self.key   = k
    self.debug = debug
  ...
  def random_key(self, numbytes):
    return b64encode(bytes([random.randint(0, 127) for _ in range(numbytes)])).decode('UTF-8')
